...
\begin{figure}[h]
	...
	\begin{subfigure}[t]{0.45\textwidth}
 		\includegraphics[width=\textwidth]{results/aliasing_fixes/compare_normalization/no_normalization_r=0_weights=1/central_view_reconstruction2-2.png} 
  		\caption{No normalization, box-filtering off, $w \equiv 1$.}
	\end{subfigure}%
	~
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{Row-normalization, box-filtering off, $w \equiv 1$.}
	...
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{No normalization, box-filtering off, weights from normal distribution $\mathcal{N}\left(0, 0.3\right)$.}
	\end{subfigure}%
	~
	\begin{subfigure}[t]{0.45\textwidth}
		...
		\caption{No normalization, box-filtering on, weights from normal distribution.}
	\end{subfigure}%
...
\begin{enumerate}
	...
	\item	Rays from bottom-most layer, weights for intersection on layers + sensor (figure \ref{fig:intersections_and_weights_overview})
		\begin{itemize}
			\item[$-$]	Need normalization of weights along ray, cancels out sensor weights
			\item[$-$]	Require rectified images for a common focal plane
			\item[$-$]	Applying box-filter is computationally expensive
			\item[$+$]	Adjustable sampling density of rays
			\item[$+$]	Fewer camera/scene parameters required, less sensitive
		\end{itemize}
\end{enumerate}
