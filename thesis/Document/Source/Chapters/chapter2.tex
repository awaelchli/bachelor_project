...
A typical assumption is that the two planes are sampled uniformly. 
One way is to capture the light field with a grid of optical systems (e.g. cameras) corresponding to the $(u, v)$. 
The extent of this grid is called the \emph{baseline} (in vertical and horizontal direction). 
This means that only a slice of the actual light field can be caputred and the two planes are clipped, usually so that they are rectangular.
 
The samples on the $(u, v)$-plane correspond to the virtual positions of the camera. 
Properties of the light field do not change under reparameterization. 
