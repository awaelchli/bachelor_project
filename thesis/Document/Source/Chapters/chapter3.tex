...
\begin{figure}[htb]
	...
	\subfigure[]{
		\centering
		\input{../Figures/ray_casting_oblique_projection}
		\label{fig:ray_casting_oblique_projection}
	}
	\hfill
	\subfigure[]{
		\centering
		\input{../Figures/ray_casting_perspective_projection}
...
For the perspective projection, it is assumed that the \mbox{$(u, v)$-plane} (or \mbox{$u$-plane} here) is at depth $Z_u$, with a distance $z_0 - Z_u$ from the image plane.
Again, let $m = m(i, k)$ be the index that identifieds the ray $(u(i), s(k))$.
...
Setting $\gamma = \frac{z_0 - Z_u}{z_n - Z_u}$ for layer $n$, the formula for the intersection is
\begin{equation}
